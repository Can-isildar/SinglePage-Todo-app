        ...
        <v-card class="elevation-12 todo-card" outlined>
          <v-card-title class="text-h6 font-weight-bold" style="margin-left: 80px">
            ...
            <v-menu v-model="menu" :close-on-content-click="false" location="end">
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props" icon class="filter-icon">
          ...
          <v-list dense>
            <v-list-item-group v-if="todoItemsList">
              <v-list-item
                v-for="(todoItem, index) in filteredTodoItemsList"
                :key="index"
              ...
              >
                <v-list-item-content class="list-item-content">
                  ...
                  <template v-if="todoItem.editing">
                    <input
                      v-model="newUpdateDescription"
                      :placeholder="newUpdateTodoPlcaeHolder"
                      class="form-control updateNewTodo"
                      @keydown.enter="confirmUpdate(todoItem.id, todoItem)"
                      @keydown.esc="cancelEdit(todoItem)"
                    />
                    <v-btn
                      ><v-icon
                        class="checkButton"
                        @click="confirmUpdate(todoItem.id, todoItem)"
                        >mdi-check-circle</v-icon
                      ></v-btn
                    >
                    <v-btn @click="cancelEdit(todoItem)"
                      ><v-icon class="closeButton">mdi-close-circle</v-icon></v-btn
                    >
                  </template>
                  ...
                  <template v-else>
                    ...
                    >
                      <p class="description-text">
                        {{ `${todoItem.description.toUpperCase()}` }}
                      </p>
                    </div>
                  </template>
          ...
          <div class="addTodoItems">
            <v-text-field
              v-model="newTodoItemsDescription"
            ...
            <v-btn @click="addTodoItem" color="primary" class="todo-button"> Add </v-btn>
          </div>
        </v-card>
      </div>
    </v-container>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        if (userData) {
          const id = localStorage.getItem("todo_id");
          todoItemsList.value = await getTodoItems(id);
        } else {
          // Kullanıcı verisi bulunamadı, yönlendirme veya hata işlemleri yapılabilir.
        }
      } catch (error) {
  ...
  methods: {
    ...
    async addTodoItem() {
      ...
      if (this.newTodoItemsDescription.trim() === "") {
        this.error = "Please enter todoItem";
        setTimeout(() => {
          this.error = false;
        }, 3000);
      } else {
        ...
        await addTodoItems(todo_id, todoItem).then((val) => {
          this.todoItemsList.push({
            ...
            completed: val.todoItem.completed,
          });
        });
        this.success = true;
        setTimeout(() => {
    ...
    async removeTodoItem(id) {
      ...
      this.todoItemsList.splice(
        this.todoItemsList.findIndex((item) => item.id == id),
        1
      );
    },
...
.filter-nav button {
  ...
  font-weight: bold;
  color: antiquewhite;
  background-color: rgb(80, 80, 182);
}
