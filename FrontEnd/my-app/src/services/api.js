...
export async function getNotes() {
  return apiClient.get('/notes').then(({ data }) => data).catch((error) => {
    throw error;
  })
}

export async function addNote(note) {
  return apiClient.post('/notes', note).then(({ data }) => data).catch((error) => {
    throw error;
  })
}

export async function deleteNote(id) {
  return apiClient.delete(`/notes/${id}`).then(({ data }) => data).catch((error) => {
    throw error;
  })
}
export async function updateNote(id,note){
  return apiClient.put(`/notes/${id}`,note).then(({data})=>data).catch((error)=>{
    throw error;
  })
}

export async function addContent(id,note){
  return apiClient.patch(`notes/${id}`,note).then(({data})=>data).catch((error)=>{
    throw error;
  })
}

export async function getNoteById(id){
  return apiClient.get(`/notes/${id}`).then(({data})=>data).catch((error)=>{
    throw error;
  })
}